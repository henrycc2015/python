<template lang="html">
  <div :class="['banner',cls]" :style="{height: height}" ref="container">
    <div :class="['items',watermark?'watermark':'']" @touchstart.stop="onTouchstart" @touchmove.stop="onTouchmove" @touchend.stop="onTouchend">
      <div class="item" v-for="(item,index) in bannerItems" :ref='`bannerItem${index}`' :key="`bim${index}`">
        <img :src="item.bannerImagePath" @click="bannerGo(item)" alt="">
      </div>
    </div>
    <div class="dots" v-if="hasDots">
      <span class="dot" :class="{active: checkDotsActive(index)}" v-for="(item, index) in dots"></span>
    </div>
  </div>
</template>
...
export default {
	...
	computed: {
		bannerItems() {
			if (this.banners.length === 2) {
				return this.banners.concat(this.banners);
			}
			return this.banners;
		},
		dots() {
	...
	mounted() {
		const bgSrc =
			'https://ohc0dpsgs.qnssl.com/image/homePage/banner_default.png';
		var bgimg = new Image();
		bgimg.onload = () => {
			this.$refs.container.style.backgroundImage = `url('${bgSrc}')`;
			this.$refs.container.style.backgroundSize = '100%';
		};
		bgimg.src = bgSrc;
	},
	activated() {
	...
	methods: {
		checkDotsActive(index) {
			if (this.banners.length === 2) {
				return this.currentPageIndex % 2 === index;
			} else {
    ...
    */
		...
		resetItemPostiion() {
			for (let i = 0; i < this.bannerItems.length; i++) {
  	...
  	*/
		...
		dealIndex(i) {
			const max = this.bannerItems.length;
			if (i >= max) {
				i = 0;
			} else if (i < 0) {
				i = max - 1;
