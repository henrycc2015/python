<template>
    <div class="tetris-picker">
        <v-flexbox :gutter="0">
            <v-flexbox-item :span="columnWidth && columnWidth[index]" v-for="(one, index) in currentData" :key="index" style="margin-left:0;">
                <div class="tetris-picker-item" :id="`tetris-picker-${uuid}-${index}`"></div>
            </v-flexbox-item>
        </v-flexbox>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        getId(i) {
            return `#tetris-picker-${this.uuid}-${i}`
        },
        render(data, value) {
