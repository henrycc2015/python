...
const TEMPLATE = `<div class="dp-container">
  <div class="dp-header">
    <div class="dp-item dp-left tetris-datetime-cancel" data-role="cancel">cancel</div>
    <div class="dp-item tetris-datetime-clear" data-role="clear"></div>
    <div class="dp-item dp-right tetris-datetime-confirm" data-role="confirm">ok</div>
  </div>
  ...
  <div class="dp-foot hide" data-role="customText"></div>
</div>`
...
DatetimePicker.prototype = {
    ...
    _show(newValueMap) {
        ...
        if (this.renderInline) {
            self.container.classList.add('tetris-datetime-view')
        }
    ...
    show(value) {
        ...
        if (config.isOneInstance) {
            if (document.querySelector('#tetris-datetime-instance')) {
                return
            }
            self.willShow = true
        }
        ...
        if (self.container) {
            ...
            if (config.isOneInstance) {
                container.id = 'tetris-datetime-instance'
            }
            ...
            if (!self.renderText && !self.renderInline) {
                ...
                //设置自定义 文本内容
                if (self.config.customText) {
                    self.find('[data-role=customText]').innerText = self.config.customText
                }
            ...
            //添加自定义文本，如 长期2017-12-27
            if(self.config.customText){
                self.find('[data-role=customText]').classList.remove('hide');
                self.find('[data-role=customText]').addEventListener('click', function (e) {
                    e.preventDefault()
                    self.customContent()
                }, false)
            }
    ...
    //自定义文本
    customContent() {
        const value = this.config.customText
        this.value = value

        if (this.config.onCustomText.call(this, value) === false) {
            return
        }
