...
window.LCalendar = (function() {
    ...
    MobileCalendar.prototype = {
        init: function(params) {
            ...
            this.trigger.value = this.defaultDate;
        },
        bindEvent: function(type) {
            ...
            function popupDate(e) {
                ...
                if (_self.id){
                    _self.gearDate.id = _self.id;
                    var oldElement = document.querySelector('#' + _self.id);
                    if (oldElement){
                ...
                _self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
                    ...
                    '</div>' +
                    '</div>';
                _self.trigger.parentNode.appendChild(_self.gearDate);
                dateCtrlInit();
                ...
                _self.gearDate.addEventListener('click', function(e) {
                    if (e.target.classList.contains('gearDate')){
            ...
            function dateCtrlInit() {
                ...
                if (/^\d{4}-\d{1,2}-\d{1,2}$/.test(_self.trigger.value)) {
                    ...
                    var rs = _self.defaultDate.match(/(^|-)\d{1,4}/g);
                    dateArr.yy = rs[0] - _self.minY;
                    dateArr.mm = rs[1].replace(/-/g, "") - 1;
                    dateArr.dd = rs[2].replace(/-/g, "") - 1;
                } else {
            ...
            function closeMobileCalendar(e) {
                ...
                if (_self.id){
                    document.querySelector('#' + _self.id).style.display = 'none';
                }
                else {
            ...
            function finishMobileDate(e) {
                ...
                var result = (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd;
                _self.trigger.value = result;
                if (_self.funcOK){
                    _self.funcOK(result);
                }
                closeMobileCalendar(e);
            ...
            function finishMobileYM(e) {
                ...
                var date_mm = parseInt(Math.round(_self.gearDate.querySelector(".date_mm").getAttribute("val"))) + 1;
                date_mm = date_mm > 9 ? date_mm : '0' + date_mm;
                var result = (date_yy % passY + _self.minY) + "-" + date_mm;
                _self.trigger.value = result;
                if (_self.funcOK){
                    _self.funcOK(result);
                }
            ...
            function finishMobileDateTime(e) {
                ...
                var result = (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd + " " + (time_hh.length < 2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm;
                _self.trigger.value = result;
                if (_self.funcOK){
                    _self.funcOK(result);
                }
            ...
            function finishMobileTime(e) {
                ...
                var time_mm = parseInt(Math.round(_self.gearDate.querySelector(".time_mm").getAttribute("val")));
                time_mm = time_mm > 9 ? time_mm : '0' + time_mm;
                var result = (time_hh.length < 2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm;
                _self.trigger.value = result;
                if (_self.funcOK){
                    _self.funcOK(result);
                }
